...
// PDF asset: add `declare module "*.pdf" { const src: string; export default src; }`
// to a src/vite-env.d.ts (or globals.d.ts) file, then this import works cleanly.
// For now we cast via require to avoid the "no type declaration" TS error.
const resumePdf = new URL("./assets/VaidehiGoruputiMarch2026Resume.pdf", import.meta.url).href;

// ── Framer Motion cubic-bezier ease must be a 4-element tuple, not number[] ───
...
interface CursorHandlers { onCursorEnter: () => void; onCursorLeave: () => void; }
...
// ── animation helpers ──────────────────────────────────────────────────────────
// `ease` typed as the required 4-tuple so Framer Motion's Variants type is satisfied
...
// ── useRevealProps — ONLY call this at the TOP of a component function ─────────
// Never call it inline inside JSX (e.g. {...useRevealProps()}) — that violates
// the Rules of Hooks. Each call site below is its own top-level hook call.
function useRevealProps(delay = 0) {
  ...
  return { ref, initial: "hidden" as const, animate: (inView ? "show" : "hidden") , variants };
}
...
function ProjectCard({ num, title, desc, tags, results, wide }: ProjectCardProps) {
  ...
  return (
    <motion.div
      ref={ref} initial="hidden" animate={controls} variants={fadeUp}
      onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{
        gridColumn: wide ? "1 / -1" : undefined,
        background: "#0d1117",
        border: `1px solid ${hovered ? "rgba(56,189,248,0.3)" : "rgba(255,255,255,0.07)"}`,
  ...
  return (
    <motion.div ref={ref} initial="hidden" animate={controls} variants={fadeUp}
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: ".5rem" }}>
        {items.map((item, i) => (
          <motion.span key={i}
            whileHover={{ y: -2, background: "rgba(56,189,248,0.1)", borderColor: "rgba(56,189,248,0.3)", color: "#38bdf8" }}
...
function ExpItem({ role, period, company, bullets, accentColor = "#38bdf8", glowColor = "rgba(56,189,248,0.5)", delay = 0 }: ExpItemProps) {
  const { ref, controls } = useReveal();
  // Variants typed explicitly so `show` key satisfies Framer Motion's Variant type
  const expVariants: Variants = {
    show: { opacity: 1, x: 0, transition: { duration: 0.6, delay } },
  };
  return (
    <motion.div ref={ref} initial={{ opacity: 0, x: -20 }} animate={controls} variants={expVariants}
...
function Counter({ target, suffix }: { target: number; suffix: string }) {
  ...
  return (
    <div ref={ref} style={{ fontFamily: "'Syne', sans-serif", fontSize: "3rem", fontWeight: 800, background: "linear-gradient(135deg,#38bdf8,#818cf8)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
      {count}{suffix}
    </div>
  );
}

// ── Section components — each calls useRevealProps() at its own top level ──────
// This is the correct pattern: hooks must be called unconditionally at the top
// of a React function, never inline inside JSX spread expressions.

function WorkSection() {
  const p0 = useRevealProps();
...
function ContactSection({ onCursorEnter, onCursorLeave }: CursorHandlers) {
  const p0 = useRevealProps();
  const p1 = useRevealProps(0.1);
  const p2 = useRevealProps(0.15);
  ...
  return (
    <>
        ...
        <ul style={{ display: "flex", gap: "2.5rem", listStyle: "none" }}>
          {navLinks.map((href, i) => (
            <li key={i} onMouseEnter={cursorEnter} onMouseLeave={cursorLeave}>
              <a href={href}
                style={{ fontSize: ".83rem", fontWeight: 400, letterSpacing: ".08em", textTransform: "uppercase", color: "#64748b", textDecoration: "none" }}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 1, background: "rgba(255,255,255,0.07)", borderTop: "1px solid rgba(255,255,255,0.07)", borderBottom: "1px solid rgba(255,255,255,0.07)", position: "relative", zIndex: 2 }}>
        {[
          { t: 5,  s: "+", l: "Years of Experience" },
          { t: 25, s: "%", l: "Performance Improvement" },
          { t: 30, s: "%", l: "Error Reduction" },
          { t: 5,  s: "+", l: "Enterprise Domains" },
        ].map((item, i) => (
      ...
      {/* SECTIONS — each is its own component so hooks run at the top level */}
      <WorkSection />
      <SkillsSection />
      <ExperienceSection />
      <ContactSection onCursorEnter={cursorEnter} onCursorLeave={cursorLeave} />
